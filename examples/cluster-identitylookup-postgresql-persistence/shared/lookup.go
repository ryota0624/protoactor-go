...
import (
	...
	ppsql "github.com/asynkron/protoactor-go/persistence/postgresql"
	ppsqlddl "github.com/asynkron/protoactor-go/persistence/postgresql/ddl"
...
func NewLockUp(actorSystem *actor.ActorSystem) (cluster.IdentityLookup, *ppsql.Provider, func()) {
	...
	postgresLookup.Init()
	return cluster.NewIdentityStorageLookup(postgresLookup), ppsql.NewProvider(pool, actorSystem, 1000), func() {
...
func prepareTables(ctx context.Context, connString string) error {
	...
	queries := []string{
		ddl.ActivationsSql,
		ddl.SpawnLocksSql,
		ppsqlddl.JournalsSQL,
		ppsqlddl.SnapshotsSQL,
	}
